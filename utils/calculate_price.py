...
def calculate_price(product_order_obj, include_initial_order=False, **kwargs):
    company_qs = product_order_obj.company
    total_price = 0.0
    ...
    for ordered_item in ordered_items_qs:
        discount_qs = ordered_item.product_size.product_color.product.discount
        item_price = ordered_item.quantity * ordered_item.product_size.product_color.product.price
        ...
        if discount_qs:
            ...
            if valid_discount_qs:
                if valid_discount_qs.discount_type == 'PERCENTAGE':
                    discount_amount_without_promo_code += (ordered_item.product_size.product_color.product.discount.amount/hundred)*item_price
                else:
                    discount_amount_without_promo_code += ordered_item.product_size.product_color.product.discount.amount
