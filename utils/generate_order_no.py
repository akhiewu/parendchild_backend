...
def generate_order_no(company_id:int, order_qs: ProductOrder = None):
    ...
    if order_qs:
        last_order_qs = ProductOrder.objects.filter(
            company_id=company_id).exclude(pk=order_qs.pk).order_by('-created_at').first()
    else:
        last_order_qs = ProductOrder.objects.filter(
            company_id=company_id).order_by('-created_at').first()
